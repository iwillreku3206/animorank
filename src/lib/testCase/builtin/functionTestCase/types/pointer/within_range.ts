import type { JsonValue } from '@zenstackhq/orm';
import { WithinRangeOperator } from '../../operators/within_range';
import { OperatorType } from '../../operatorType';
import { TypeValue } from '../../typeValue.svelte';
import type { Pointer } from '.';

/** Compares pointer values by dereferencing through the target type (legacy parity). */
export class WithinRangePointer extends OperatorType<WithinRangeOperator, Pointer> {
  public async compare(expected: TypeValue<Pointer>, actual: TypeValue<Pointer>): Promise<boolean> {
    const innerExpected = TypeValue.assumedValid(expected.type.targetType, expected.value as JsonValue);
    const innerActual = TypeValue.assumedValid(actual.type.targetType, actual.value as JsonValue);
    return new WithinRangeOperator(this.options).compare(innerExpected, innerActual);
  }
}
