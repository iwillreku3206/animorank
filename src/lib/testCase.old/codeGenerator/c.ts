...
export class CCodeGenerator extends CodeGenerator {
  ...
  private generateComparison(
    ...
    switch (operator) {
      ...
      case 'LESS_THAN':
        comparisonCompute = expected.constructLessThanCheck(comparisonSymbol, resolvedSymbol, expectedSymbol);
        break;
      case 'GREATER_THAN':
        comparisonExpression = `!${comparisonExpression}`;
      // eslint-disable-next-line no-fallthrough
      case 'LESS_THAN_EQUAL':
        comparisonCompute = expected.constructLessThanEqualCheck(comparisonSymbol, resolvedSymbol, expectedSymbol);
        break;
      case 'NOT_EQUAL':
        comparisonExpression = `!${comparisonExpression}`;
      // eslint-disable-next-line no-fallthrough
      case 'EQUAL':
        comparisonCompute = expected.constructEqualityCheck(comparisonSymbol, resolvedSymbol, expectedSymbol);
        break;
      case 'WITHIN_RANGE':
