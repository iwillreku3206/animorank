...
class CPointer extends LanguageType<PointerValue> {
  ...
  private resolveInnerLanguage(ptr: Pointer): LanguageType<unknown> | null {
    try {
      ...
      console.warn(`Failed to resolve inner language for pointer target: ${ptr.getTargetType()}`, e);
      return null;
    }
  }
  ...
  public constructLessThanEqualCheck(resultSymbol: string, symbolA: string, symbolB: string): string {
    return this.innerLanguage.constructLessThanEqualCheck(resultSymbol, `*${symbolA}`, `*${symbolB}`);
  }
  ...
  public constructWithinRangeCheck(resultSymbol: string, symbol: string, actualSymbol: string, range: string): string {
    return this.innerLanguage.constructWithinRangeCheck(resultSymbol, `*${symbol}`, `*${actualSymbol}`, range);
  }
}
