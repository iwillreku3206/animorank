...
export class FileLogger extends Logger {
  ...
  constructor(module: string, timeFormat?: Intl.DateTimeFormat) {
    ...
    this.timeFormat = timeFormat || new Intl.DateTimeFormat('en', {
      year: '2-digit',
      month: '2-digit',
      day: '2-digit',
      hour: '2-digit',
  ...
  protected async log(message: Loggable): Promise<void> {
    ...
    await fs.appendFile(process.env.FILE_LOGGER_FILE || '/tmp/animorank.log', Buffer.from(`[${LogLevelNames[message.level]}] [${formatted}] [${this.module}] ${message.message}\n`), 'utf8')
  }
}
