					...
					<div class="flex flex-col h-full w-full">
						<div class="flex flex-row p-2">
							...
							<div class="flex flex-row gap-2 ml-auto">
								<button class="btn btn-sm" onclick={() => (toggleTestResults = !toggleTestResults)}>
								...
								<button class="btn btn-sm" onclick={handleReset}>Reset Code</button>
								<button class="btn btn-sm" onclick={handleRun}>Run</button>
								<button class="btn btn-sm bg-[#006239] text-white hover:bg-[#004327]">Submit</button
								>
							</div>
						</div>

						<CodeEditor bind:code language={data.problem.language.toLowerCase()} />
					</div>
				</Pane>
				{#if toggleTestResults}
					<Pane>
						<TestCaseDisplay tests={testCaseResults} {toggleTestResults} />
					</Pane>
				{/if}
			</Splitpanes>
		</Pane>
