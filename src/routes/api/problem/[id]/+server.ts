...
export const PUT: RequestHandler = async ({ locals, request, params }) => {
  ...
  const { success, error: zodError, data } = await putValidator.safeParseAsync(await request.json())
  if (!success) return error(400, zodError)

  await db.problem.update({ where: { id: params.id, problem_set: { owner_id: session.user.id } }, data })
