...
export const PUT: RequestHandler = async ({ locals, params, request }) => {
  ...
  const currentTestCase = await db.problemTestCase.findUnique({ where: { id: params.id, problem: { problem_set: { owner_id: session.user.id } } } })
  if (!currentTestCase) return error(404, "Not found") // should return 404 if not owned by user to hide existence of such an object

  const { success, error: zodError, data } = await {
